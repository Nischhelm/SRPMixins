...
public abstract class SRPCommandRootMixin {
    ...
    @WrapOperation(
            method = "execute",
            at = @At(value = "INVOKE", target = "Ljava/lang/StringBuilder;append(I)Ljava/lang/StringBuilder;" ,ordinal = 1)
    )
    private StringBuilder srpmixins_modifyMobCap(
            StringBuilder instance,
            int mobCap,
            Operation<StringBuilder> original,
            @Local(name = "count") int count,
            @Local(argsOnly = true) ICommandSender sender
    ){
        WorldMobCapEvent event = new WorldMobCapEvent(sender.getEntityWorld(), ParasiteCreatureType.PARASITE, Collections.emptySet(), 0, 0, mobCap);
